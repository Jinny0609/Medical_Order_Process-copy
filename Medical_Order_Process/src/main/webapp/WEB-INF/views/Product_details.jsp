	...
	<div class="I_container">
		...
		<div class="B_itembox">
			<c:forEach items="${Product_detail}" var="detail">
				<div>
					<img class="I_img" src="${detail.image_path}" alt="의료용품이미지">
				</div>
				<div class="I_Detail">
					<div class="I_title">
					...
					<input type="hidden" name="product_name"
						value="${detail.product_name}"> <input type="hidden"
						name="product_price" value="${detail.product_price}"> <input
						type="hidden" name="image_path" value="${detail.image_path}">
					<div class="I_BT">
						<button type="button" id="add-to-cart-btn"
							class="btn btn-outline-primary"
							data-user-id="<%=session.getAttribute("user_id")%>">장바구니
							담기</button>
						<button type="button" id="buy-now-btn" class="btn btn-primary"
							data-user-id="<%=session.getAttribute("user_id")%>">바로
							구매</button>
					</div>
				</div>
			</c:forEach>
